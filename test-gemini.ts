...
/**
 * 実験用スクリプト
 * 使用方法: npx tsx test-gemini.ts <画像1のパス> <画像2のパス>
 * 
 * 例: npx tsx test-gemini.ts ./test-images/image1.jpg ./test-images/image2.jpg
 */
async function main() {
    const args = process.argv.slice(2);

    if (args.length < 2) {
        console.error('使用方法: npx tsx test-gemini.ts <画像1のパス> <画像2のパス>');
        console.error('例: npx tsx test-gemini.ts ./test-images/image1.jpg ./test-images/image2.jpg');
        process.exit(1);
    }

    const [image1Path, image2Path] = args;
    ...
    try {
        ...
        console.log(`✅ 画像1を読み込みました: ${image1Path} (${image1Buffer.length} bytes)`);
        console.log(`✅ 画像2を読み込みました: ${image2Path} (${image2Buffer.length} bytes)`);
        console.log('');
        ...
        console.log(`   画像内にラベル: ${result.has_label_in_image ? 'あり' : 'なし'}`);
        console.log(`   認証マーク: ${result.certifications.length > 0 ? result.certifications.join(', ') : 'なし'}`);
        console.log(`   ハラム成分: ${result.ingredients_flags.haram.length > 0 ? result.ingredients_flags.haram.join(', ') : 'なし'}`);
        console.log(`   疑わしい成分: ${result.ingredients_flags.suspect.length > 0 ? result.ingredients_flags.suspect.join(', ') : 'なし'}`);
        console.log(`   アレルギー成分: ${result.allergens.found.length > 0 ? result.allergens.found.join(', ') : 'なし'}`);
